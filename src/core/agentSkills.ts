...
function workflowSkillMarkdown(config: AgentSkillConfig): string {
  return [
    ...
    "6. Use `jumpspace link suggest <id> --json` for working-tree candidates, or add `--since <ref>` / repeated `--path <path>` for a baseline or explicit candidate set before changing task links. Changed-file status is candidate context, not proof, so inspect `evidence.path_terms`, `evidence.basename_terms`, `evidence.identifier_terms`, `evidence.content_terms`, `evidence.phrase_matches`, coverage, and `rejected_candidates` before trusting a suggestion. `NO_SOURCE_EVIDENCE` means the file was considered and deliberately not linked. Run `jumpspace link eval --json` after scorer or PR-assistant ranking changes; when a real repo exposes a bad ranking, add a JSON fixture and run `jumpspace link eval --file <fixture.json> --json`. Apply with `jumpspace link update <id> --dry-run --json` first, then without `--dry-run` only after the change is intended.",
...
function reviewSkillMarkdown(config: AgentSkillConfig): string {
  return [
    ...
    "6. Use `jumpspace link suggest <id> --json` for working-tree candidates, or add `--since <ref>` / repeated `--path <path>` for a baseline or explicit candidate set; changed-file status alone is not proof.",
...
function handoffSkillMarkdown(config: AgentSkillConfig): string {
  return [
    ...
    "5. Preview explicit gaps with `jumpspace link update <id> --gap \"...\" --dry-run --json`, then rerun without `--dry-run` to persist them.",
