...
describe("addJumpspaceSkills", () => {
  it("installs Codex guidance and a repo-local skill idempotently", async () => {
    ...
    expect(skill).toContain("jumpspace task bootstrap context");
    expect(skill).toContain("jumpspace task pr comment --since <ref>");
    ...
    expect(skill).toContain("jumpspace task link eval --json");
    expect(skill).toContain("jumpspace task link eval --file <fixture.json> --json");
    ...
    expect(skill).toContain("jumpspace task history --task <id> --json");
    expect(skill).toContain("jumpspace task handoff --task <id> --json");
    expect(skill).toContain("handoff packet status");
    expect(skill).toContain("jumpspace schema show task.handoff --json");
    ...
    expect(workSkill).toContain("jumpspace task work <id> --json");
    expect(workSkill).toContain("jumpspace task verify <id>");
  ...
  it("installs Claude guidance and preserves existing Claude content", async () => {
    ...
    const intentReviewSkill = await fs.readFile(path.join(root, ".claude/skills/jumpspace-intent-review/SKILL.md"), "utf8");
    ...
    expect(claude).toContain("@.claude/skills/jumpspace-intent-review/SKILL.md");
    ...
    expect(intentReviewSkill).toContain("name: jumpspace-intent-review");
    expect(intentReviewSkill).toContain("jumpspace intent validate --since <ref> --json");
    expect(intentReviewSkill).toContain("jumpspace intent verify --since <ref> --json");
    ...
    expect(handoffSkill).toContain("jumpspace task handoff --task <id> --json");
  ...
  it("installs the intent-review pipeline skill with the reference workflow", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "jumpspace-skill-"));

    const result = await addJumpspaceSkills(root, ["codex"], {
      skills: ["jumpspace-intent-review"],
    });

    expect(result.skills).toEqual(["jumpspace-workflow", "jumpspace-intent-review"]);
    expect(result.files).toEqual([
      expect.objectContaining({ agent: "codex", path: "AGENTS.md", action: "created" }),
      expect.objectContaining({
        agent: "codex",
        skill: "jumpspace-workflow",
        path: ".codex/skills/jumpspace-workflow/SKILL.md",
        action: "created",
      }),
      expect.objectContaining({
        agent: "codex",
        skill: "jumpspace-intent-review",
        path: ".codex/skills/jumpspace-intent-review/SKILL.md",
        action: "created",
      }),
    ]);
    ...
    expect(agents).toContain("@.codex/skills/jumpspace-workflow/SKILL.md");
    expect(agents).toContain("@.codex/skills/jumpspace-intent-review/SKILL.md");
    expect(agents).not.toContain("@.codex/skills/jumpspace-work/SKILL.md");
    expect(intentReviewSkill).toContain("Review matched repo-local intents against branch diffs");
    expect(intentReviewSkill).toContain("possible_violation");
    expect(intentReviewSkill).toContain("Prefer false negatives over false positives.");
  });
});
