...
export function renderPlanPacket(state: ExecutionState): string {
  ...
  return [
    ...
    "- Run `jumpspace scan` after editing the docs.",
    "",
  ].join("\n");
}
...
export function renderExecutionPacket(state: ExecutionState, options: { forced?: boolean } = {}): string {
  ...
  return [
    ...
    "## Agent workflow",
    "",
    "- Treat this packet and `jumpspace context` as the implementation source of truth.",
    ...
    "- Run relevant tests, then `jumpspace scan` and `jumpspace audit` before finishing.",
    "- In the final response, mention the task ID and verification performed.",
    "",
    "## Suggested commands",
    "",
    "```bash",
    `jumpspace context ${task.id}`,
    "npm test",
    "jumpspace scan",
    "jumpspace audit",
    "```",
