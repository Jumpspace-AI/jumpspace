...
Use this workflow by default for repo questions, feature work, branch review, first-graph bootstrap, and handoffs. The user should not have to say "use Jumpspace" on every request once this file is installed. If Jumpspace evidence is missing, weak, or stale, say that clearly and then fall back to normal code search.
...
For repeated integrations, prefer the published SDK contracts over ad hoc JSON parsing: TypeScript can import from `jumpspace/sdk`, Python can use the dataclass package in `sdk/python/jumpspace_sdk`, and package consumers can pin generated schema artifacts from `jumpspace/schemas/catalog.json` plus `jumpspace/schemas/<name>.schema.json`.
